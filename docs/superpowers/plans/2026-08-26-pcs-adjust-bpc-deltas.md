...
| `CheckSrcAbs()` / `CheckDstAbs()` / `AdjustPCS()` | **Retained, deprecated, and no longer called from anywhere in IccProfLib.** All 16 guarded call sites were deleted in Task 5 (`ea2ca3b2`); the definitions stay at `IccProfLib/IccCmm.cpp:1877,1911,1806` | third-party `CIccXform` subclasses only -- they are `protected`, so an out-of-tree `Apply()` can still call them | Now genuinely "deprecated, third-party subclasses only," which was **not** true of the tree this matrix originally described. Such a subclass double-applies: `CIccPcsXform` has already performed the adjustment at the connection. The spectral-edge case the previous row cited stopped firing under S5 of the spectral white point work (the base predicates answer false at a spectral port), and `spectralTrailingEdgeNoLongerAdjustsInsideApply()` now pins the opposite of what its predecessor pinned. **One reachable exception survives the deletion as a behaviour change** -- an MCS destination port fed by an `IIccAdjustPCSXform` hint; see `pcsAdjustHintReachesANonPcsPort()` and `task-5-report.md` §6. |
