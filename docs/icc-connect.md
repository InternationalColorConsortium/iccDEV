...
returned `nullptr`, pass a `std::string*` through the `pErrorMsg`
...
`CreateStandard` or the three-argument `CreateSearch`. When `nThreads == 0`
the factory uses
...
A threaded wrapper hides a named-color CMM, so `GetNamedCmm()` returns
`nullptr`. `GetSearchCmm()` unwraps a threaded search CMM;
`GetThreadedCmm()` and `IsThreaded()` expose the wrapper directly.
