...
`container_changed` remains available to risk reporting and scope
classification; it selects the full matrix without adding a Docker-specific
aggregate result. Dockerfile, Docker dependency, packaged MCP, and
Docker-workflow changes are published through the canonical `ci-docker`
workflow on `master`. Only `ci-docker` publishes images.
