...
- Use `latest` for maintainer container validation, resolve it to a digest
  before starting the container job, and record both that digest and the image
  source revision. Select a full-SHA tag only for an explicit reproducibility
  need; do not hardcode one as a durable workflow default.
