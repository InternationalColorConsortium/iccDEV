#!/usr/bin/env bash
#################################################################################
# .github/ci/quality-assurance/scripts/icc_ci_tool_path_exercise.sh
...
# ICC CI path exerciser for:
#   - iccApplyProfiles
#   - iccApplySearch
#
# Aligned with observed CI command shapes and documented CLI usage.
#
# Default:
#   --tool=both
#   --mutations=1000
#   runs export commands and replays generated configs via -cfg when possible.
...
PCC_OVERRIDE=""
ICC_OVERRIDE=""
INCLUDE_CASE_VARIANTS=0
INCLUDE_RISKY_SCALARS=1
INCLUDE_ENV_MUTATIONS=1
TOOL_SCOPE="both"          # profiles | search | both

usage() {
  cat <<'EOF'
Usage:
  ./icc_ci_tool_path_exercise.sh [options]

Modes:
  --count                         Print computed mutation-space size and exit.
  --generate FILE                 Write generated export commands to FILE and exit.
  --dry-run                       Print commands instead of running.

Tool selection:
  --tool profiles                 Exercise iccApplyProfiles only.
  --tool search                   Exercise iccApplySearch only.
  --tool both                     Exercise both. Default.
...
Generation:
  ...
  --case-variants                 Also test -EMBEDDED/-PCC spelling variants for iccApplyProfiles.
  --no-risky-scalars              Keep output scalar args closer to CI defaults.
...
Overrides:
  --pcc-directory DIR             Replace PCC/profile-condition files with *.icc from DIR, cycling by index.
  --pcc-profile FILE              Replace PCC/profile-condition file with FILE.
  --icc-profile FILE              Replace final output ICC profile before final rendering_intent when applicable.
...
Examples:
  ./icc_ci_tool_path_exercise.sh --count
  ./icc_ci_tool_path_exercise.sh --tool profiles --mutations=1000 --jobs 8
  ./icc_ci_tool_path_exercise.sh --tool search --mutations=1000 --jobs 8
  ./icc_ci_tool_path_exercise.sh --tool both --mutations=5000 --jobs 8
  ./icc_ci_tool_path_exercise.sh --generate ci-path-cmds.txt --mutations=max
EOF
}
...
# ----------------------------------------------------------------------
# iccApplySearch setup
# ----------------------------------------------------------------------

SEARCH_ENCODINGS=("0" "0:4" "0:6:10" "1" "1:4" "2" "3" "4" "5" "6")
SEARCH_INTERPS=(0 1)
SEARCH_DEBUG_PREFIX=("" "-debugcalc ")

# Per usage:
# data_file_path encoding[:precision[:digits]] interpolation
#   {-ENV:tag value} profile1_path intent1
#   {{-ENV:tag value} middle_profile_path mid_intent}
#   {-ENV:tag value} profile2_path intent2
#   -INIT init_intent2 {pcc_path1 weight1 ...}
#
# CI command:
# iccApplySearch -exportcfganddata config/cmykGraysEst.json Results/cmykGraysRef.txt 0 1
#   ICC/Spec380_10_730-D50_2deg.icc 3
#   ICC/Lab_float-D50_2deg.icc 3
#   ICC/CMYK_Hybrid_Profile.icc 10003
#   -INIT 3
...
#
SEARCH_DATA_FILES=(Results/cmykGraysRef.txt)
SEARCH_PROFILE1=(ICC/Spec380_10_730-D50_2deg.icc)
SEARCH_MIDDLE=(ICC/Lab_float-D50_2deg.icc)
SEARCH_PROFILE2=(ICC/CMYK_Hybrid_Profile.icc)

SEARCH_INTENT1=(3 103 10003 10103)
SEARCH_MID_INTENT=(3 43 93 103 10003 10043 10093 10103)
SEARCH_INTENT2=(3 43 93 103 10003 10043 10093 10103)
SEARCH_INIT_INTENT=(0 1 2 3 40 41 42 43 90 91 92 93 100 101 102 103 10000 10003 10103)

SEARCH_PCC_SETS=(
  "ICC/Lab_float-D50_2deg.icc 1 ICC/Lab_float-D93_2deg-MAT.icc 1 ICC/Lab_float-F11_2deg-MAT.icc 1 ICC/Lab_float-IllumA_2deg-MAT.icc 1"
  "ICC/Lab_float-D50_2deg.icc 1"
  "ICC/Lab_float-D93_2deg-MAT.icc 1 ICC/Lab_float-F11_2deg-MAT.icc 1"
  "ICC/Lab_float-IllumA_2deg-MAT.icc 1"
  "ICC/Lab_float-D50_2deg.icc 0.5 ICC/Lab_float-D93_2deg-MAT.icc 0.25 ICC/Lab_float-F11_2deg-MAT.icc 0.25"
  "ICC/Lab_float-D50_2deg.icc 0 ICC/Lab_float-D93_2deg-MAT.icc 1"
  "ICC/Lab_float-D50_2deg.icc 2"
)

SEARCH_ENV_BEFORE_P1=("" "-ENV:bkgX 0.0985 " "-ENV:abcd 1234 ")
SEARCH_ENV_BEFORE_MID=("" "-ENV:bkgY 0.159 " "-ENV:0ni? 1 ")
SEARCH_ENV_BEFORE_P2=("" "-ENV:bkgZ 0.122 " "-ENV:bkgX 0 -ENV:bkgY 0 -ENV:bkgZ 0 ")
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-risky-scalars)
      INCLUDE_RISKY_SCALARS=0
      SAMPLE_ENCODINGS=(1 2)
      COMPRESSIONS=(1)
      PLANARS=(0)
      DST_EMBED_ICC=(0 1)
      INTERPOLATIONS=(0 1)
      SEARCH_ENCODINGS=("0" "0:4" "1" "3")
      SEARCH_INIT_INTENT=(3 10003)
      shift
      ;;
    --no-env-mutations)
      INCLUDE_ENV_MUTATIONS=0
      SEARCH_ENV_BEFORE_P1=("")
      SEARCH_ENV_BEFORE_MID=("")
      SEARCH_ENV_BEFORE_P2=("")
      shift
      ;;
...
case "$TOOL_SCOPE" in
  profiles|search|both) ;;
  *) die "--tool must be profiles, search, or both" ;;
esac
...
profile_intent_values() {
  case "$1" in
    ...
    PREVIEW) printf '%s\n' "${INTENTS_PREVIEW[@]}" ;;
    MCS) printf '%s\n' "${INTENTS_MCS[@]}" ;;
    *) die "unknown profile intent set: $1" ;;
  esac
}
...
template_stage_count() {
  ...
  for stage in "${stages[@]}"; do
    ...
    rest="${stage#*:}"
    case "$kind" in
      embedded) intent_set="${rest%%:*}" ;;
      profile|envprofile) intent_set="${stage##*:}" ;;
      *) die "bad stage kind: $kind" ;;
    esac
    mapfile -t vals < <(profile_intent_values "$intent_set")
    n="${#vals[@]}"
    count=$((count * n))
  done
  echo "$count"
...
PROFILES_MAX="$(profiles_max_count)"
SEARCH_MAX="$(search_max_count)"

case "$TOOL_SCOPE" in
  profiles) MAX_MUTATIONS="$PROFILES_MAX" ;;
  search) MAX_MUTATIONS="$SEARCH_MAX" ;;
  both) MAX_MUTATIONS=$((PROFILES_MAX + SEARCH_MAX)) ;;
esac

if [[ "$MODE" == "count" ]]; then
  echo "$MAX_MUTATIONS"
  echo "profiles=$PROFILES_MAX" >&2
  echo "search=$SEARCH_MAX" >&2
  exit 0
fi
...
rewrite_search_cmd() {
  local idx="$1" cmd="$2"
  # For iccApplySearch, --pcc-* overrides the pcc_pathN entries after -INIT, preserving weights.
  if [[ "$PCC_OVERRIDE" == file:* ]]; then
    local repl="${PCC_OVERRIDE#file:}"
    cmd="$(awk -v repl="$repl" '
      {
        init=0
        for (i=1;i<=NF;i++) {
          if ($i=="-INIT") { init=i; i++; continue }
          if (init && i>init+1 && ((i-init)%2)==0) $i=repl
        }
        print
      }' <<< "$cmd")"
  elif [[ "$PCC_OVERRIDE" == dir:* ]]; then
  ...
  if [[ -n "$ICC_FILE_OVERRIDE" ]]; then
    # final target profile is profile2 immediately before intent2 and before -INIT.
    cmd="$(awk -v repl="$ICC_FILE_OVERRIDE" '
      {
        for (i=1;i<=NF;i++) {
          if ($i=="-INIT" && i>=3) {
            $(i-2)=repl
...
# ----------------------------------------------------------------------
# iccApplyProfiles emission
# ----------------------------------------------------------------------

emit_profile_stage_chains() {
  local spec="$1" embedded_sw="$2" pcc_sw="$3" current="$4"
  local first remaining kind rest intent_set profile env_idx pcc_key pcc vals val new_current
  first="${spec%%;*}"
  remaining=""
  [[ "$spec" == *";"* ]] && remaining="${spec#*;}"
  ...
  case "$kind" in
    embedded)
      ...
      mapfile -t vals < <(profile_intent_values "$intent_set")
      for val in "${vals[@]}"; do
        pcc="$(profile_pcc_path "$pcc_key")"
        if [[ -n "$pcc" ]]; then
          new_current="${current}${embedded_sw} ${val} ${pcc_sw} ${pcc} "
        else
          new_current="${current}${embedded_sw} ${val} "
        fi
        if [[ -n "$remaining" ]]; then emit_profile_stage_chains "$remaining" "$embedded_sw" "$pcc_sw" "$new_current"; else printf '%s\n' "$new_current"; fi
      done
      ;;
    profile)
      profile="${rest%:*}"
    ...
    envprofile)
      ...
      ;;
    *) die "bad profile stage kind: $kind" ;;
  esac
}

emit_profiles() {
  local start_idx="$1" wanted="$2" start_at="$3"
  local idx="$start_idx" emitted=0 tmpl name src dstbase spec enc comp planar dst_embed interp thread embedded_sw pcc_sw chain tag cmd
  for tmpl in "${PROFILE_TEMPLATES[@]}"; do
                ...
                for embedded_sw in "${EMBEDDED_SWITCHES[@]}"; do
                  for pcc_sw in "${PCC_SWITCHES[@]}"; do
                    while IFS= read -r chain; do
                      ...
                      tag="$(printf '%05d_profiles_%s_e%s_c%s_p%s_emb%s_i%s' "$idx" "$name" "$enc" "$comp" "$planar" "$dst_embed" "$interp")"
                      ...
                      rewrite_profile_cmd "$idx" "$cmd"
                      emitted=$((emitted + 1))
                    done < <(emit_profile_stage_chains "$spec" "$embedded_sw" "$pcc_sw" "")
                  done
                done
              done
            done
...
# ----------------------------------------------------------------------
# iccApplySearch emission
# ----------------------------------------------------------------------

emit_search() {
  local start_idx="$1" wanted="$2" start_at="$3"
  local idx="$start_idx" emitted=0 data enc interp dbg env1 p1 i1 envm mid im env2 p2 i2 init pccset tag cmd

  for data in "${SEARCH_DATA_FILES[@]}"; do
    for enc in "${SEARCH_ENCODINGS[@]}"; do
      for interp in "${SEARCH_INTERPS[@]}"; do
        for dbg in "${SEARCH_DEBUG_PREFIX[@]}"; do
          for env1 in "${SEARCH_ENV_BEFORE_P1[@]}"; do
            for p1 in "${SEARCH_PROFILE1[@]}"; do
              for i1 in "${SEARCH_INTENT1[@]}"; do
                for envm in "${SEARCH_ENV_BEFORE_MID[@]}"; do
                  for mid in "${SEARCH_MIDDLE[@]}"; do
                    for im in "${SEARCH_MID_INTENT[@]}"; do
                      for env2 in "${SEARCH_ENV_BEFORE_P2[@]}"; do
                        for p2 in "${SEARCH_PROFILE2[@]}"; do
                          for i2 in "${SEARCH_INTENT2[@]}"; do
                            for init in "${SEARCH_INIT_INTENT[@]}"; do
                              for pccset in "${SEARCH_PCC_SETS[@]}"; do
                                idx=$((idx + 1))
                                [[ "$idx" -lt "$start_at" ]] && continue
                                [[ "$emitted" -ge "$wanted" ]] && return 0

                                tag="$(printf '%05d_search_enc%s_i%s_i1%s_mid%s_i2%s_init%s' "$idx" "${enc//:/_}" "$interp" "$i1" "$im" "$i2" "$init")"
                                cmd="iccApplySearch ${dbg}-exportcfganddata config/ci_search_${tag}.json ${data} ${enc} ${interp} ${env1}${p1} ${i1} ${envm}${mid} ${im} ${env2}${p2} ${i2} -INIT ${init} ${pccset}"
                                cmd="$(sed 's/[[:space:]][[:space:]]*/ /g; s/[[:space:]]$//' <<< "$cmd")"
                                cmd="$(rewrite_search_cmd "$idx" "$cmd")"
                                printf '%s > Results/ci_search_%s.txt\n' "$cmd" "$tag"
                                emitted=$((emitted + 1))
                              done
                            done
                          done
...
emit_all() {
  ...
  case "$TOOL_SCOPE" in
    profiles)
      emit_profiles 0 "$wanted" "$start_at"
      ;;
    search)
      emit_search 0 "$wanted" "$start_at"
      ;;
    both)
      # Global numbering: profiles occupy 1..PROFILES_MAX; search follows.
      local emitted_profiles=0 remaining="$wanted"
      if [[ "$start_at" -le "$PROFILES_MAX" ]]; then
        emitted_profiles=$(( PROFILES_MAX - start_at + 1 ))
        [[ "$emitted_profiles" -gt "$wanted" ]] && emitted_profiles="$wanted"
        emit_profiles 0 "$emitted_profiles" "$start_at"
        remaining=$((wanted - emitted_profiles))
        [[ "$remaining" -le 0 ]] && return 0
        emit_search "$PROFILES_MAX" "$remaining" $((PROFILES_MAX + 1))
      else
        emit_search "$PROFILES_MAX" "$wanted" "$start_at"
      fi
      ;;
  esac
}
...
echo "Computed mutation space: $MAX_MUTATIONS" >&2
echo "profiles=$PROFILES_MAX search=$SEARCH_MAX scope=$TOOL_SCOPE" >&2
...
if [[ "$MODE" == "generate" ]]; then
  ...
  emit_all "$MUTATIONS" "$START_AT" > "$OUT_FILE"
