#!/bin/bash
###############################################################################
# iccDEV composite tag recursion-depth regression tests (#1437)
###############################################################################
#
# Compiles .github/ci/regression/tagcomposite-recursion-depth.cpp against the
# build's IccProfLib and verifies that deeply-nested tagArrayType / tagStructType
# chains are rejected by the CompositeDepthGuard without stack exhaustion, while
# a legitimate shallow chain still reads. Before the fix the deep chain crashes
# (stack-overflow at IccTagComposite.cpp:1350); ASan surfaces it as a finding.
#
...
run_recursion_depth_helper() {
  local name="tagcomposite-recursion-depth"
  local helper_cpp="$REPO_ROOT/.github/ci/regression/tagcomposite-recursion-depth.cpp"
  ...
  if grep -q "ERROR: AddressSanitizer\\|runtime error:\\|stack-overflow" "$run_log" 2>/dev/null; then
    fail_case "$name" "sanitizer finding while reading nested composite tags"
    sed -n '1,80p' "$run_log"
    rm -f "$helper_bin"
    return
  fi
  ...
  rm -f "$helper_bin"
  pass_case "$name" "nested composite tag chains rejected without stack exhaustion"
}

echo "=== composite tag recursion-depth regression (#1437) ==="

run_recursion_depth_helper

echo "composite tag recursion-depth regression: $PASS passed, $FAIL failed, $TOTAL total"
