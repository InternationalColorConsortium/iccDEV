#!/bin/bash
###############################################################################
# iccDEV Interp6d non-finite float-cast regression tests (#1504)
###############################################################################
#
# Compiles .github/ci/regression/interp6d-nan-cast.cpp against the build's
# IccProfLib and verifies that CIccCLUT::Interp6d() sanitises a non-finite 6th
# grid coordinate before the (icUInt32Number) cast. Before the fix a NaN/Inf in
# srcPixel[5] reaches the unguarded cast (UBSan: "nan is outside the range of
# representable values of type 'unsigned int'" at IccTagLut.cpp:3290); the
# float-cast-overflow sanitizer surfaces it as a finding. After the fix Interp6d
# clamps g5 to 0 like g0..g4 and returns finite output.
#
...
run_interp6d_helper() {
  local name="interp6d-nan-cast"
  local helper_cpp="$REPO_ROOT/.github/ci/regression/interp6d-nan-cast.cpp"
  ...
  if grep -q "ERROR: AddressSanitizer\\|runtime error:\\|outside the range" "$run_log" 2>/dev/null; then
    fail_case "$name" "sanitizer finding while interpolating a non-finite 6th coordinate"
    sed -n '1,80p' "$run_log"
    rm -f "$helper_bin"
    return
  fi
  ...
  rm -f "$helper_bin"
  pass_case "$name" "Interp6d sanitised the non-finite 6th coordinate without a cast trap"
}

echo "=== Interp6d non-finite float-cast regression (#1504) ==="

run_interp6d_helper

echo "Interp6d non-finite float-cast regression: $PASS passed, $FAIL failed, $TOTAL total"
