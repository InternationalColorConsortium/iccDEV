/**
 * IccVizMath — small shared colour-math helpers for the visualization tools.
 *
 * These are the pure scalar conversions that both IccVizModel (the data-first
 * model) and iccProfileVisualize (the PDF/TIFF renderer) need: XYZ→xy
 * chromaticity projection and the Kang et al. planckian-locus approximation.
 *
 * They live in one header so the math — especially approxPlanck()'s polynomial
 * constants — has a single source and can't drift between the two tools. The
 * functions are header-only (inline) and depend only on the ICC scalar types,
 * so the header carries no link dependency.
 */
